...
const ALL_PME_OPTIONS = [
  'PME_counter-l_plain_overflow.json',
  'PME_counter-l_plain_straight.json',
  'PME_counter-l_sink-01_overflow.json',
  'PME_counter-l_sink-01_straight.json',
  'PME_counter-l_sink-02_overflow.json',
  'PME_counter-l_sink-02_straight.json',
  'PME_counter-top_corner_overflow.json',
  'PME_counter-top_corner_straight.json',
  'PME_counter-top_overflow_plain.json',
  'PME_counter-top_overflow_sink.json',
  'PME_counter-top_straight_plain.json',
  'PME_counter-top_straight_sink.json',
  'PME_sink.json',
]
...
export default function App() {
  const [pmeOptions, setPmeOptions] = useState<string[]>([])
  ...
  useEffect(() => {
    ...
    const loadRootPmeOptions = async () => {
      const childIds = new Set<string>()
      const validPmes = new Set<string>()

      await Promise.all(
        ALL_PME_OPTIONS.map(async (fileName) => {
          const response = await fetch(`/configs/${fileName}`)
          if (!response.ok) return
          const raw = (await response.json()) as RawJson
          if (String(raw.type).toLowerCase() !== 'parametricelement') return
          validPmes.add(fileName)

          const attachmentPoints = (Array.isArray(raw.attachmentPoints)
          ...
          for (const apRaw of Array.isArray(attachmentPoints) ? attachmentPoints : []) {
            if (!apRaw || typeof apRaw !== 'object') continue
            const childRaw = (apRaw as RawJson).child as RawJson | undefined
            if (!childRaw || typeof childRaw !== 'object') continue
            const childId = typeof childRaw.elementId === 'string' ? childRaw.elementId.trim() : ''
            if (!childId) continue
            childIds.add(`${childId}.json`)
          }
        }),
      )
      ...
      const rootOptions = [...validPmes].filter((fileName) => !childIds.has(fileName)).sort()
      setPmeOptions(rootOptions)
      setSelectedPme((previous) => (previous && rootOptions.includes(previous) ? previous : rootOptions[0] ?? ''))
    }

    void loadRootPmeOptions()
  ...
  return (
    <main className="app-shell">
      ...
      <PMESelector options={pmeOptions} selected={selectedPme} isLoading={isLoadingConfig} onSelect={setSelectedPme} />
